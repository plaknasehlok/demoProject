...
public class UserController {
	...
	/*
	 * @RequestMapping("/logout") public ModelAndView indexagain(HttpServletRequest
	 * request) {
	 * 
	 * String uname=(String)request.getSession().getAttribute("user_id");
	 * System.out.println("inside /logout method in controller");
	 * System.out.println(uname); //request.getSession().invalidate(); return new
	 * ModelAndView("login", new HashMap<>()); }
	 */
	...
	/*
	 * @RequestMapping("/accountdetails") public ModelAndView fundstransfer() {
	...
	/*
	 * @RequestMapping("/viewrequest") public ModelAndView view() {
	 * 
	 * Map<String, Object> map = new HashMap<>();
	 * 
	 * List<List<String>> listOfProfiles = new ArrayList<List<String>>();
	 * 
	 * // Retrieval from db List<User_Profiles> profiles = service1.allRequests();
	 * 
	 * for (User_Profiles up : profiles) { List<String> profileDetails = new
	 * ArrayList<String>();
	 * 
	 * String user_profile_id_string = up.getUser_profile_id().toString();
	 * profileDetails.add(user_profile_id_string);
	 * profileDetails.add(up.getAadhar_no()); profileDetails.add(up.getF_name());
	 * profileDetails.add(up.getL_name()); profileDetails.add(up.getM_name());
	 * profileDetails.add(up.getFathers_name()); long mobile_no_long =
	 * up.getMobile_no(); String mobile_no_string = Long.toString(mobile_no_long);
	 * profileDetails.add(mobile_no_string); profileDetails.add(up.getEmail_id());
	 * 
	 * profileDetails.add(up.getDob().toString());
	 * 
	 * profileDetails.add(up.getRes_addr()); profileDetails.add(up.getPer_addr());
	 ...
	 * }
	 * 
	 * // System.out.print(listOfProfiles); map.put("Requests", listOfProfiles);
	 * 
	 ...
	 * createProcess(@RequestParam String acc_no,@RequestParam Integer balance){
	 * Account_Details user=service2.updateFunds(acc_no,balance);
	 * Map<String,Object>map=new HashMap<>(); map.put("acc_no",user.getAcc_no());
	 * map.put("balance",user.getBalance()); ModelAndView mv=new
	 * ModelAndView("userupdated",map); return mv; }
	 */

	/*
	 * @RequestMapping("/createprocess") public ModelAndView
	 * createProcess(@RequestParam String sender_acc_no,@RequestParam String
	 * receiver_acc_no, @RequestParam Integer amount){ Transaction_Details
	 * user=ss.createtrans(sender_acc_no, receiver_acc_no, amount);
	...
	public ModelAndView accountstatementsent(@RequestParam Date fromdate, @RequestParam Date todate, HttpSession session, HttpServletRequest request) {
		/*
		 * String uname=(String)request.getSession().getAttribute("user_id");
		 * System.out.println(uname);
		 */
		...
		/*//Received
		List<List<String>> listOfStatementsReceived = new ArrayList<List<String>>();
		List<Transaction_Details> statementsreceived = service3.allStatementsReceived(currentaccno,fromdate,todate);
		
		for (Transaction_Details tdr : statementsreceived) {
			List<String> statementDetailsReceived = new ArrayList<String>();
			System.out.println("inside for");
			String transaction_id_string = tdr.getTrans_id().toString();
			statementDetailsReceived.add(transaction_id_string);
			statementDetailsReceived.add(tdr.getSender_acc_no());
			statementDetailsReceived.add(tdr.getReceiver_acc_no());
			String transaction_amount_string = tdr.getAmount().toString();
